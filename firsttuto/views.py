...
from firsttuto.utils import get_max_order
...
class TaskList(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return UserTask.objects.filter(user=user).order_by("order")
...
def add_task(request):
    ...
    if not UserTask.objects.filter(user=request.user, task=task).exists():
        UserTask.objects.create(
            user=request.user, task=task, order=get_max_order(request.user)
        )
...
def delete_task(request, pk):
    ...
    UserTask.objects.filter(user=request.user, pk=pk).delete()
...
def search_task(request):
    ...
    results = Task.objects.filter(description__icontains=search_text).exclude(
        description__in=usertasks.values_list("task__description", flat=True)
    )[:10]
...
@login_required
def items(request):
    items = request.POST.getlist("task_order")
    for i, item in enumerate(items):
        task = UserTask.objects.get(pk=item)
...
def reorder(user):
    tasks = UserTask.objects.filter(user=user).order_by("order")
    if not tasks:
        return
    for i, task in enumerate(tasks):
        UserTask.objects.filter(user=user, task=task.task).update(order=i+1)
